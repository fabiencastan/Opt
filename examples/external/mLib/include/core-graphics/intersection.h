...
{\
		...
		const vec3<FloatType>& anchor1, const vec3<FloatType>* axesScaled1) 
	{
		...
		//vec3<FloatType> const& C0 = box0.center;
		//vec3<FloatType> const* A0 = &box0.axis[0];
		//vec3<FloatType> const& E0 = box0.extent;
		//vec3<FloatType> const& C1 = box1.center;
		//vec3<FloatType> const* A1 = &box1.axis[0];
		//vec3<FloatType> const& E1 = box1.extent;

		std::array<vec3<FloatType>, 3> A0;	
		A0[0] = axesScaled0[0].getNormalized();	
		A0[1] = axesScaled0[1].getNormalized();	
		A0[2] = axesScaled0[2].getNormalized();
		vec3<FloatType> E0 = vec3<FloatType>(axesScaled0[0].length(), axesScaled0[1].length(), axesScaled0[2].length()) / 2;
		vec3<FloatType> C0 = anchor0 + (axesScaled0[0] + axesScaled0[1] + axesScaled0[2])/2;

		std::array<vec3<FloatType>, 3> A1;	
		A1[0] = axesScaled1[0].getNormalized();	
		A1[1] = axesScaled1[1].getNormalized();	
		A1[2] = axesScaled1[2].getNormalized();
		vec3<FloatType> E1 = vec3<FloatType>(axesScaled1[0].length(), axesScaled1[1].length(), axesScaled1[2].length()) / 2;
		vec3<FloatType> C1 = anchor1 + (axesScaled1[0] + axesScaled1[1] + axesScaled1[2]) / 2;;

		const FloatType cutoff = (FloatType)1 - result.epsilon;
		bool existsParallelPair = false;

		// Compute difference of box centers.
		vec3<FloatType> D = C1 - C0;
		...
		for (int i = 0; i < 3; ++i)
		{
			dot01[0][i] = vec3<FloatType>::dot(A0[0], A1[i]);
		...
		dotDA0[0] = vec3<FloatType>::dot(D, A0[0]);
		r = std::abs(dotDA0[0]);
		...
		for (int i = 0; i < 3; ++i)
		{
			dot01[1][i] = vec3<FloatType>::dot(A0[1], A1[i]);
		...
		dotDA0[1] = vec3<FloatType>::dot(D, A0[1]);
		r = std::abs(dotDA0[1]);
		...
		for (int i = 0; i < 3; ++i)
		{
			dot01[2][i] = vec3<FloatType>::dot(A0[2], A1[i]);
		...
		dotDA0[2] = vec3<FloatType>::dot(D, A0[2]);
		r = std::abs(dotDA0[2]);
		...
		r = std::abs(vec3<FloatType>::dot(D, A1[0]));
		...
		r = std::abs(vec3<FloatType>::dot(D, A1[1]));
		...
		r = std::abs(vec3<FloatType>::dot(D, A1[2]));
